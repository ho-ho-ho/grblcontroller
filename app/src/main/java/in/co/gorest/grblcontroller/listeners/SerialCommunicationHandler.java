...
public abstract class SerialCommunicationHandler extends Handler {
    ...
    private final String[] startUpCommands = {GrblUtils.GRBL_BUILD_INFO_COMMAND,
            GrblUtils.GRBL_VIEW_SETTINGS_COMMAND, GrblUtils.GRBL_VIEW_PARSER_STATE_COMMAND,
            GrblUtils.GRBL_VIEW_GCODE_PARAMETERS_COMMAND};
    ...
    protected boolean onSerialRead(String message) {
        ...
        if (GrblUtils.isGrblOkMessage(message)) {
            ...
            if (machineStatus.getVerboseOutput()) {
            ...
            GrblSettingMessageEvent settingMessageEvent = new GrblSettingMessageEvent(GrblSettings,
                    message);
            ...
            machineStatus.setCompileTimeOptions(
                    GrblUtils.getCompileTimeOptionsFromString(buildOptions));
            EventBus.getDefault().post(new ConsoleMessageEvent(message));
        ...
        } else if (machineStatus.getCustomStartUpString().length() > 0 && message.equalsIgnoreCase(
                machineStatus.getCustomStartUpString())) {
            EventBus.getDefault().post(new ConsoleMessageEvent(message));
            MachineStatusListener.BuildInfo buildInfo = new MachineStatusListener.BuildInfo(1.1,
                    'f');
            ...
            if (buildInfo.versionDouble >= Constants.MIN_SUPPORTED_VERSION) {
                ...
                String messageNotSupported = GrblController.getInstance()
                        .getString(R.string.text_grbl_unsupported,
                                String.valueOf(Constants.MIN_SUPPORTED_VERSION));
    ...
    private synchronized void updateMachineStatus(String statusMessage) {
        ...
        for (String part : statusMessage.substring(0, statusMessage.length() - 1).split("\\|")) {
            ...
            if (part.startsWith("<")) {
                ...
                MPos = GrblUtils.getPositionFromStatusString(statusMessage,
                        GrblUtils.machinePattern);
                ...
                if (overrideParts.length == 3) {
                    machineStatus.setOverridePercents(Integer.parseInt(overrideParts[0]),
        ...
        if (WPos == null && MPos != null) {
            WPos = new Position(MPos.getCordX() - WCO.getCordX(), MPos.getCordY() - WCO.getCordY(),
                    MPos.getCordZ() - WCO.getCordZ());
        }
        if (MPos == null && WPos != null) {
            MPos = new Position(WPos.getCordX() + WCO.getCordX(), WPos.getCordY() + WCO.getCordY(),
                    WPos.getCordZ() + WCO.getCordZ());
        }
        ...
        if (!enabledPinsChanged) {
            machineStatus.setEnabledPins("");
        }

        if (hasOverrides) {
            if (!accessoryStatesChanged) {
                machineStatus.setAccessoryStates("");
            }
        }
