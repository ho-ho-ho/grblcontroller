...
public class FileStreamerIntentService extends IntentService {
    ...
    private final BlockingQueue<Integer> completedCommands = new ArrayBlockingQueue<>(
            Constants.DEFAULT_SERIAL_RX_BUFFER);

    private static volatile boolean isServiceRunning = false;
    private static volatile boolean shouldContinue = true;

    public synchronized static boolean getIsServiceRunning() {
        return isServiceRunning;
    }

    private synchronized static void setIsServiceRunning(boolean running) {
        isServiceRunning = running;
    }

    public synchronized static boolean getShouldContinue() {
        return shouldContinue;
    }

    public synchronized static void setShouldContinue(boolean b) {
        shouldContinue = b;
    }
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (fileSenderListener.getGcodeUri() == null) {
            EventBus.getDefault()
                    .post(new UiToastEvent(getString(R.string.text_no_gcode_file_selected), true,
                            true));
            return;
        }
                ...
                public void run() {
                    int elapsedTimeSeconds =
                            (int) (System.currentTimeMillis()
                                    - fileSenderListener.getJobStartTime()) / 1000;
                    fileSenderListener.setElapsedTime(
                            String.format(Locale.US, "%02d:%02d:%02d", elapsedTimeSeconds / 3600,
                                    (elapsedTimeSeconds % 3600) / 60, (elapsedTimeSeconds % 60)));
                }
            }, 0, 1000);
        ...
        if (isCheckMode) {
            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.N_MR1) {
                startForeground(Constants.FILE_STREAMING_NOTIFICATION_ID,
                        getNotification(getString(R.string.text_file_checking_started),
                                fileSenderListener.getGcodeFileName()));
            }

            if (defaultConnectionType != null && defaultConnectionType.equals(
            ...
            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.N_MR1) {
                startForeground(Constants.FILE_STREAMING_NOTIFICATION_ID,
                        getNotification(getString(R.string.text_file_streaming_started),
                                fileSenderListener.getGcodeFileName()));
            }
        ...
        StreamingCompleteEvent streamingCompleteEvent = new StreamingCompleteEvent(
                "Streaming Completed");
        ...
        streamingCompleteEvent.setTimeMillis(
                fileSenderListener.getJobEndTime() - fileSenderListener.getJobStartTime());
    ...
    private void startStreaming(int statusUpdateInterval) {
        ...
        try {
            br = new BufferedReader(new InputStreamReader(
                    getContentResolver().openInputStream(fileSenderListener.getGcodeUri())));
            int linesSent = 0;
    ...
    private void waitUntilBufferRunOut(boolean dwell) {
        if (dwell) {
            streamLine(new GcodeCommand("G4P0.01"));
        }
