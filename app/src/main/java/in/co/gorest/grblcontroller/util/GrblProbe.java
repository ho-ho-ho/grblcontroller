...
public class GrblProbe {
  ...
  private double getSettingProbingPlateOffsetX() {
    return Double.parseDouble(sharedPref.getString(
        GrblController.getInstance().getString(R.string.preference_probing_touch_plate_x_offset),
        "10.0"
    ));
  }
  ...
  private double getSettingProbingPlateOffsetY() {
    return Double.parseDouble(sharedPref.getString(
        GrblController.getInstance().getString(R.string.preference_probing_touch_plate_y_offset),
        "10.0"
    ));
  }
    ...
    if (stateConfig.type != ProbeType.ToolLengthOffset) {
      ...
      if (stateConfig.referenceZ) {
        ...
        EventBus.getDefault().post(new UiToastEvent(
            GrblController.getInstance().getString(R.string.text_probe_success_with_tlo)));
      }
    }
