...
public class GrblProbe {
    ...
    public static GrblProbe getInstance(Context context,
            BaseFragment.OnFragmentInteractionListener handler) {
        if (grblProbe == null) {
            grblProbe = new GrblProbe(
                    handler,
                    EnhancedSharedPreferences.getInstance(context,
    ...
    private double getSettingProbingDistanceXY() {
        return Double.parseDouble(sharedPref.getString(
                GrblController.getInstance().getString(
                        R.string.preference_probing_touch_distance_xy),
                "15.0"
        ));
    }
    ...
    private double getSettingProbingDepthXY() {
        return Double.parseDouble(sharedPref.getString(
                GrblController.getInstance().getString(R.string.preference_probing_touch_depth_xy),
                "10.0"
        ));
    }
    ...
    private double getSettingProbingFeedRate() {
        return Double.parseDouble(sharedPref.getString(
                GrblController.getInstance().getString(R.string.preference_probing_feed_rate),
                "50.0"
        ));
    }
    ...
    private double getSettingProbingToolDiameter() {
        return Double.parseDouble(sharedPref.getString(
                GrblController.getInstance().getString(R.string.preference_probing_tool_diameter),
                "6.0"
        ));
    }
    ...
    private double getSettingProbingPlateOffsetX() {
        return Double.parseDouble(sharedPref.getString(
                GrblController.getInstance().getString(
                        R.string.preference_probing_touch_plate_x_offset),
                "10.0"
        ));
    }
    ...
    private double getSettingProbingPlateOffsetY() {
        return Double.parseDouble(sharedPref.getString(
                GrblController.getInstance().getString(
                        R.string.preference_probing_touch_plate_y_offset),
                "10.0"
        ));
    }
    ...
    private double getSettingProbingPlateOffsetZ() {
        return Double.parseDouble(sharedPref.getString(
                GrblController.getInstance().getString(R.string.preference_probing_plate_thickness),
                "10.0"
        ));
    }
    ...
    // the probe first travels to the waypoint in starting Z height and then lowers by touchDepth
    // for the actual probing towards the probe target.
    private double getWaypointXY(Pair<Axis, Direction> axisDir) {
        if (stateConfig.type == ProbeType.Center
                && stateConfig.centerDir == CenterDirection.Inside) {
    ...
    private double getBackoffXY(Pair<Axis, Direction> axisDir) {
        if (stateConfig.type == ProbeType.Center
            ...
            double touchPlateOffset = getSettingProbingDistanceXY();
            double offset =
                    axisDir.second.isPositive() ? (touchPlateOffset + 1) : -(touchPlateOffset + 1);
            return getStartForAxis(axisDir.first) + offset;
        }
    }
    ...
    private void probeXYAxis(Pair<Axis, Direction> axisDir) {
        ...
        grblHandler.onGcodeCommandReceived(
                "G38.3" + axisDir.first.name() + offset + "F" + getSettingProbingFeedRate());
    }
    ...
    private void onProbeXYAxis(Pair<Axis, Direction> axisDir, double probedPosition) {
        ...
        double offset =
                getTouchPlateOffsetForAxis(axisDir.first) + getSettingProbingToolDiameter() / 2.0;
    ...
    private void probeZAxis(Pair<Axis, Direction> axisDir) {
        grblHandler.onGcodeCommandReceived("G91");
        grblHandler.onGcodeCommandReceived(
                "G38.3Z-" + getSettingProbingDepthXY() + "F" + getSettingProbingFeedRate());
    }
        ...
        if (stateConfig.type != ProbeType.ToolLengthOffset) {
            ...
            if (stateConfig.referenceZ) {
                ...
                EventBus.getDefault().post(new UiToastEvent(
                        GrblController.getInstance().getString(
                                R.string.text_probe_success_with_tlo)));
            }
        }
    ...
    public void probe(Configuration config) {
        ...
        if (config.type == ProbeType.ToolLengthOffset) {
            ...
            //TODO: check if one axis is present with both Plus and Minus directions and return
            // if true
        }
    ...
    public void onGrblProbeEvent(GrblProbeEvent event) {
        ...
        if (axisDir.first != Axis.Z) {
            onProbeXYAxis(axisDir,
                    (axisDir.first == Axis.X) ? event.getProbeCordX() : event.getProbeCordY());
        } else {
